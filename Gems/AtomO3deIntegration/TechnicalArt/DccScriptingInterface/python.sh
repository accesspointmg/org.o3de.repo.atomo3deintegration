...
PYTHON_VENV=$O3DE_3RDPARTY_PATH/venv/$ENGINE_ID
...
then
    ...
    echo "Try running $O3DE_ENGINE_PATH/python/get_python.sh first."
    exit 1
fi
...
then
    ...
    echo "Run $O3DE_ENGINE_PATH/python/get_python.sh to update."
    exit 1
fi
...
PYTHON_LIB_PATH=$(awk -F ' = ' '/home/ {print $2}' $O3DE_3RDPARTY_PATH/venv/$ENGINE_ID/pyvenv.cfg | sed 's/python\/bin/python\/lib/g')
